...
void push_rpc_confirmation (JOB_REF_ARG (C), int confirm) {
  ...
  /*
   * In padded modes, avoid emitting the 5-byte quick-ack packet
   * (0xdd + 4 bytes).
   *
...
int f_parse_option (int val) {
  ...
  case 2018: {
    int x = atoi (optarg);
    if (x < 0 || x > 4) {
      kprintf ("'--tls-fake-ticket-records' must be in range [0, 4], got '%s'\n", optarg);
      usage ();
      return 2;
    }
    tcp_rpc_set_tls_fake_ticket_records (x);
    break;
  }
  default:
    return -1;
...
void mtfront_prepare_parse_options (void) {
  ...
  parse_option ("tls-fake-ticket-records", required_argument, 0, 2018, "append extra opaque TLS records after server flight (0..4, default 0)");
